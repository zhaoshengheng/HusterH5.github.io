...
const identities = {
  highschool: {
    ...
    title: '未来 HUSTer 探索路线', desc: '用地标提前预习一所大学的学习、生活与理想。',
...
function updateIdentityPreview() {
  ...
  $('#routeDesc').textContent = item.desc;
...
function updateRouteCard() {
  ...
  $('#routeCard').innerHTML = `
    <span>${String(item.id).padStart(2, '0')} / ${item.campus} / ${item.group}</span>
    <h3>${item.name}</h3>
    <p>${item.intro}</p>
    <button class="small-btn" id="enterLandmark">查看这个地标</button>
  `;
...
function updateLandmark() {
  ...
  $('#identityCopy').textContent = item.copies[currentIdentity];
  ...
  $('#unlockBtn').textContent = unlocked.has(item.id) ? '已点亮' : '点亮坐标';
  $('#unlockBtn').disabled = unlocked.has(item.id);
...
function setViewMode(mode) {
  activeView = mode;
  if (mode === 'now') setCompare(100);
  if (mode === 'future') setCompare(0);
  if (mode === 'compare') setCompare($('#compareRange').value || 48);
}
  ...
  if (item.mode === 'timegate') {
    module.innerHTML = `
      <div class="interaction-card timegate-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>TIME SCRUB</span></div>
        <div class="timegate-cards">
          <div class="timegate-card"><b>记忆</b><small>旧照片与入园第一眼</small></div>
          <div class="timegate-card"><b>现在</b><small>真实地标与校园日常</small></div>
          <div class="timegate-card"><b>未来</b><small>智慧导览与精神坐标</small></div>
        </div>
        <input class="timeline-range" id="timelineRange" type="range" min="0" max="2" step="1" value="1" aria-label="拖动时间轴" />
        <div class="timeline-row"><span>回望</span><span>此刻</span><span>未来</span></div>
        <p class="module-copy" id="moduleCopy">${item.moduleText}</p>
      </div>
    `;
    const texts = ['回望：旧照片感、校门初见和校史线索叠加，适合校友与新生情绪入口。', item.moduleText, '未来：将全息导览、智慧校园与精神坐标叠加，强化“现在—未来”的主题转化。'];
    $('#timelineRange').addEventListener('input', e => {
      const v = Number(e.target.value);
      $('#moduleCopy').textContent = texts[v];
      setCompare(v === 0 ? 72 : v === 2 ? 26 : 48);
      popSeasonDots();
    });
  } else if (item.mode === 'book') {
    module.innerHTML = `
      <div class="interaction-card book-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>PAGE FLIP</span></div>
        <div class="book-stack" id="bookStack">
          <button class="book-page"><b>纸页</b><br>知识在安静里沉淀</button>
          <button class="book-page"><b>云端</b><br>AI 把检索变成对话</button>
          <button class="book-page"><b>长期主义</b><br>未来学习仍需要专注</button>
        </div>
        <p class="module-copy" id="moduleCopy">${item.moduleText}</p>
      </div>
    `;
    const lines = ['翻到纸页：专业不是标签，而是长期积累。', '翻到云端：AI 是工具，问题意识才是入口。', '翻到长期主义：图书馆连接个人学习与大学精神。'];
    $$('.book-page').forEach((btn, idx) => btn.addEventListener('click', () => {
      $('#moduleCopy').textContent = lines[idx];
      setCompare(idx === 0 ? 68 : idx === 1 ? 24 : 48);
      btn.animate([{ transform: 'rotateY(0deg)' }, { transform: 'rotateY(-12deg)' }, { transform: 'rotateY(0deg)' }], { duration: 420, easing: 'ease-out' });
    }));
  } else if (item.mode === 'nature') {
    const seasons = ['春', '夏', '秋', '冬'];
    module.innerHTML = `
      <div class="interaction-card nature-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>SEASONS</span></div>
        <div class="season-wheel">
          ${seasons.map(s => `<button class="season-btn ${s === currentSeason ? 'active' : ''}" data-season="${s}">${s}</button>`).join('')}
        </div>
        <p class="module-copy" id="moduleCopy">${item.moduleText}</p>
      </div>
    `;
    const seasonCopy = {
      '春': '春：新芽、社团招新、录取季，把未来放进第一阵风。',
      '夏': '夏：树荫、蝉声、湖面光，把校园生活拉长。',
      '秋': '秋：落叶、晚风、旧照片，适合给校友做情绪回望。',
      '冬': '冬：雾气、灯光、自习路，适合做安静而温暖的未来校园。'
    };
    const seasonCompare = { '春': 42, '夏': 36, '秋': 62, '冬': 52 };
    $$('.season-btn').forEach(btn => btn.addEventListener('click', () => {
      currentSeason = btn.dataset.season;
      $$('.season-btn').forEach(b => b.classList.toggle('active', b === btn));
      $('#moduleCopy').textContent = seasonCopy[currentSeason];
      setCompare(seasonCompare[currentSeason]);
      popSeasonDots();
    }));
  } else if (item.mode === 'classroom') {
    module.innerHTML = `
      <div class="interaction-card classroom-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>LIGHT UP</span></div>
        <div class="window-grid" id="windowGrid">${Array.from({ length: 32 }, (_, i) => `<button class="window-dot ${i % 7 === 0 || i === 18 ? 'on' : ''}" aria-label="教室灯光"></button>`).join('')}</div>
        <p class="module-copy" id="moduleCopy">${item.moduleText}</p>
      </div>
    `;
    $$('.window-dot').forEach(dot => dot.addEventListener('click', () => {
      dot.classList.toggle('on');
      const count = $$('.window-dot.on').length;
      $('#moduleCopy').textContent = `已点亮 ${count} 间教室/实验格：每一格都代表一次讨论、实验或灵感。`;
      setCompare(Math.max(20, 72 - count));
    }));
  } else if (item.mode === 'medicine') {
    module.innerHTML = `
      <div class="interaction-card medicine-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>PULSE</span></div>
        <div class="pulse-strip" id="pulseStrip"></div>
        <p class="module-copy" id="moduleCopy">${item.moduleText}</p>
      </div>
    `;
    $('#pulseStrip').addEventListener('click', () => {
      $('#moduleCopy').textContent = '仁心、严谨、责任、生命温度，是医学未来最重要的底色。';
      setCompare(34);
      popSeasonDots();
    });
  } else {
    module.innerHTML = `
      <div class="interaction-card science-module">
        <div class="module-head"><h3>${item.moduleTitle}</h3><span>STARS</span></div>
...
function bindEvents() {
  ...
  $('#campusAccordion').addEventListener('click', e => {
    ...
    if (target) {
      selectLandmark(target.dataset.index);
      return;
    }
  ...
  $('#restart').addEventListener('click', () => { identityChosen = false; renderIdentities(); updateIdentityPreview(); goTo(0); });
...
function updateStageScale() {
  const scale = Math.min(window.innerWidth / 640, window.innerHeight / 1030);
